...
describe('component utils', () => {
  ...
  test('destroy backend element', () => {
    const compDef = componentSpace.define().registerComponent()
    const comp = glassEasel.Component.createWithContext('root', compDef.general(), domBackend)
    const shadowRoot = comp.getShadowRoot()!

    const t1 = shadowRoot.createTextNode('a')
    expect(t1.getBackendContext()).toBe(domBackend)
    expect(t1.getBackendElement()).toBeTruthy()
    const c1 = shadowRoot.createComponentByDef('c', compDef)
    expect(c1.getBackendContext()).toBe(domBackend)
    expect(c1.getBackendElement()).toBeTruthy()
    c1.appendChild(t1)
    const sr1 = c1.getShadowRoot()!
    expect(sr1.getBackendContext()).toBe(domBackend)
    const n1 = sr1.createNativeNode('n')
    expect(n1.getBackendContext()).toBe(domBackend)
    expect(n1.getBackendElement()).toBeTruthy()
    sr1.appendChild(n1)
    const v1 = shadowRoot.createVirtualNode()
    expect(v1.getBackendContext()).toBe(domBackend)
    v1.appendChild(c1)
    shadowRoot.appendChild(v1)

    v1.destroyBackendElementOnSubtree()
    expect(t1.getBackendElement()).toBeNull()
    expect(c1.getBackendContext()).toBeNull()
    expect(c1.getBackendElement()).toBeNull()
    expect(sr1.getBackendContext()).toBeNull()
    expect(n1.getBackendContext()).toBeNull()
    expect(n1.getBackendElement()).toBeNull()
    expect(v1.getBackendContext()).toBeNull()
  })

  test('destroy backend element on removal', () => {
    const compDef = componentSpace.define().registerComponent()
    const comp = glassEasel.Component.createWithContext('root', compDef.general(), domBackend)
    const shadowRoot = comp.getShadowRoot()!

    const t1 = shadowRoot.createTextNode('a')
    t1.destroyBackendElementOnRemoval()
    expect(t1.getBackendContext()).toBe(domBackend)
    expect(t1.getBackendElement()).toBeTruthy()
    const c1 = shadowRoot.createComponentByDef('c', compDef)
    c1.destroyBackendElementOnRemoval()
    expect(c1.getBackendContext()).toBe(domBackend)
    expect(c1.getBackendElement()).toBeTruthy()
    c1.appendChild(t1)
    const sr1 = c1.getShadowRoot()!
    expect(sr1.getBackendContext()).toBe(domBackend)
    const n1 = sr1.createNativeNode('n')
    n1.destroyBackendElementOnRemoval()
    expect(n1.getBackendContext()).toBe(domBackend)
    expect(n1.getBackendElement()).toBeTruthy()
    sr1.appendChild(n1)
    const v1 = shadowRoot.createVirtualNode()
    v1.destroyBackendElementOnRemoval()
    expect(v1.getBackendContext()).toBe(domBackend)
    v1.appendChild(c1)
    shadowRoot.appendChild(v1)

    shadowRoot.removeChild(v1)
    expect(t1.getBackendElement()).toBeNull()
    expect(c1.getBackendContext()).toBeNull()
    expect(c1.getBackendElement()).toBeNull()
    expect(sr1.getBackendContext()).toBeNull()
