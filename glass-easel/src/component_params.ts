 ...
 * IsAny<{}> = false
 */
export type IsAny<T> = (<S>(S: S) => S extends T ? 1 : 2) extends <R>(
  R: R
) => R extends any ? 1 : 2
  ? true
  : false;
...
export type ObserverDataPathStrings<
  T,
 ...
 */
export type GetFromDataPath<
  T,
  K extends readonly (string | number)[]
> = K extends [infer F, ...infer R extends (string | number)[]]
  ? F extends keyof T
...
export type Tag<B, T extends symbol> = [IfNeverOrAny<B, unknown>] extends [
  null | undefined
]
  ? B
...
export type HasTag<B, T extends symbol> = T extends GetTags<B>[number]
  ? true
  : false;
...
export type PropertyOption<T extends PropertyType, V> = {
  ...
  default?: () => V
  observer?:
    | ((newValue: DeepReadonly<V>, oldValue: DeepReadonly<V>) => void)
    | string
  reflectIdPrefix?: boolean
};

export type PropertyListItem<T extends PropertyType, V> =
  | T
  | PropertyOption<T, V>
...
export type DataWithPropertyValues<
...
export type TaggedMethod<Fn extends ComponentMethod> = Tag<
  Fn,
  typeof METHOD_TAG
>;

export type UnTaggedMethod<M extends TaggedMethod<any>> = UnTag<
  M,
  typeof METHOD_TAG
>;
